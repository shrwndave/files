<?php
	...
	class AccountModel extends Connector{
		...
		function updateAccount($user){//update account
			//prepare for query
			$query = "UPDATE `user_table` SET `user_fullname`='{$user['name']}',`user_email`='{$user['email']}',`user_password`='{$user['password']}' WHERE `user_id`='{$user['id']}'";
			//prepapre binding
		...
		function deleteAccount($user){//delete account
			//prepare for query
			$query = "DELETE FROM `user_table` WHERE `user_id`='{$user['id']}'";
			//prepapre binding
			$stmt = $this->conn->query($query);
			$stmt->execute();//execute query
			return $stmt->fetchall(PDO::FETCH_ASSOC);
